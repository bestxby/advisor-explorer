...
export default function KPISummary({ professors, directions }) {
  const stats = useMemo(() => {
    const avgDifficulty = (directions.reduce((sum, d) => sum + d.difficulty, 0) / directions.length).toFixed(1);
    const highRecommend = directions.filter(d => d.recommendation >= 4).length;
    const wideJobMarket = directions.filter(d => d.jobMarket === '极宽' || d.jobMarket === '宽').length;
